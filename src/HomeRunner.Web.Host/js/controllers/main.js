materialAdmin
    ...
    .controller('materialadminCtrl', function ($scope, $timeout, $state, growlService) {
        console.log($scope);

        var self = this;
        $scope.$watch(function () { return self.layoutType; }, function (current, previous) {
            $timeout(function () {
                self.sidebarToggle.left = (current == 0);
            }, 0);
        });
